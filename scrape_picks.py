...
def cmd_pull(args):
    from playwright.sync_api import sync_playwright
    bundle = {"yahoo": {}, "yahoo_pick_pct": {}, "dk": {}, "consensus": {}, "pinnacle": {}, "dvoa": {}, "ftn_proj": {}, "pulled_at": time.strftime("%Y-%m-%d %H:%M")}
    with sync_playwright() as pw:
    ...
    if key:
        try:
            bundle["dk"], bundle["consensus"], bundle["pinnacle"] = fetch_dk(key)
        except Exception as e:
    ...
    if bundle["yahoo"]:
        ...
        for src in ("dk", "consensus", "pinnacle"):
