...
def _dump_inputs(page, tag):
    try:
        info = page.eval_on_selector_all(
            "input, button, a",
            "els => els.filter(e => e.offsetParent !== null).map(e => `${e.tagName} type=${e.type||''} name=${e.name||''} id=${e.id||''} ph=${e.placeholder||''} text=${(e.innerText||'').trim().slice(0,40)}`).join('\\n')")
        Path(f"debug_ftn_login_{tag}.txt").write_text(info + "\n\n----- page text -----\n" + page.inner_text("body"))
    except Exception:
        pass


def ftn_login(ctx):
    """Sign in with FTN_EMAIL / FTN_PASSWORD. Handles a login modal or a dedicated page."""
    ...
    if not (email and pw):
        ...
        return False
    page = ctx.new_page()
    load_page(page, "https://www.ftnfantasy.com/", settle=3)
    if ftn_logged_in(page):
        page.close()
        return True
    try:
        # open the login UI: try the header "Log In" first, then the /login route
        opened = False
        for sel in ("a:has-text('Log In')", "button:has-text('Log In')", "a[href*='login']", "text=Log In"):
        ...
        _dump_inputs(page, "before")

        # email/password fields — exclude the site search box
        email_sel = ("input[type=email], input[name=email], input[id*=email i], input[autocomplete=username], "
                     "input[autocomplete=email], input[placeholder*=mail i], input[type=text]:not([name=search])")
        efield = page.locator(email_sel).filter(visible=True).first
        pfield = page.locator("input[type=password]").filter(visible=True).first
        efield.wait_for(timeout=10000)
        efield.fill(email)
        pfield.fill(pw)
        submitted = False
        for sel in ("button[type=submit]", "button:has-text('Log In')", "button:has-text('Sign In')", "button:has-text('Login')", "input[type=submit]"):
        ...
        if not ok:
            _dump_inputs(page, "after")
        page.close()
        return ok
    except Exception as e:
        print(f"ftn login: error {type(e).__name__}: {str(e).splitlines()[0]}", file=sys.stderr)
        _dump_inputs(page, "error")
        page.close()
        return False
