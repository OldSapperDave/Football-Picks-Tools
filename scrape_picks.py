...
def parse_ftn_projections(text):
    """
    Placeholder until the real table layout is known: try each line as
    'away ... home ... awayScore ... homeScore' and derive home spread = away - home;
    otherwise fall back to the generic spread parser.
    """
    games = {}
    for line in text.splitlines():
        hits = team_hits(line)
        nums = [float(x) for x in re.findall(r"(?<![\d.-])(\d{1,2}(?:\.\d+)?)(?![\d.%])", line)]
        if len(hits) >= 2 and len(nums) >= 2:
            a, h = hits[0][0], hits[1][0]
            away_pts, home_pts = nums[-2], nums[-1]
            if 0 <= away_pts <= 60 and 0 <= home_pts <= 60:
                games[f"{a}@{h}"] = round((away_pts - home_pts) * 2) / 2
    return games or parse_games(text)
...
def fetch_dk(api_key):
    ...
    r = requests.get(
        ...
        params={"apiKey": api_key, "regions": "us,eu", "markets": "spreads", "oddsFormat": "american"},
        timeout=30,
    )
    r.raise_for_status()
    ...
    dk, consensus, pinnacle, nbooks, kickoff = {}, {}, {}, {}, {}
    for ev in r.json():
    ...
    return dk, consensus, pinnacle, kickoff
...
def cmd_pull(args):
    from playwright.sync_api import sync_playwright
    bundle = {"yahoo": {}, "yahoo_pick_pct": {}, "dk": {}, "consensus": {}, "pinnacle": {}, "kickoff": {}, "dvoa": {}, "ftn_proj": {}, "pulled_at": time.strftime("%Y-%m-%d %H:%M")}
    with sync_playwright() as pw:
    ...
    if key:
        try:
            bundle["dk"], bundle["consensus"], bundle["pinnacle"], bundle["kickoff"] = fetch_dk(key)
        except Exception as e:
    ...
    if bundle["yahoo"]:
        ...
        for src in ("dk", "consensus", "pinnacle", "kickoff"):
