...
# ---------------- browser ----------------
def cookie_header_to_list(header, domain):
    out = []
...
def open_browser(pw, headless):
    ...
    if yc or fc:
        ...
        if yc:
            ctx.add_cookies(cookie_header_to_list(yc, ".yahoo.com"))
        if fc:
            ctx.add_cookies(cookie_header_to_list(fc, ".ftnfantasy.com"))
        return ctx
...
def grab_text(ctx, url, wait_for=None, name=""):
    ...
    if "sign in" in text.lower()[:3000] and "sign out" not in text.lower():
        print(f"warning: {name} page looks logged out — check the cookies secret", file=sys.stderr)
    ...
    with sync_playwright() as pw:
        ...
        try:
            for url in (base, base + "/picks", base + "/pickdistribution"):
                ...
                if "not a member of this group" in text.lower() or "sign in" in text.lower() and "sign out" not in text.lower():
                    print("yahoo: page shows a login/membership error — check the YAHOO_COOKIES secret", file=sys.stderr)
                    break
